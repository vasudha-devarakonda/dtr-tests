...
def d_loop(config, dset, G, D, d_optimizer, criterion):
    ...
    #  1A: Train D on real
    d_real_data = torch.from_numpy(dset.sample(config.minibatch_size)).cuda(0)
    d_real_decision = D(d_real_data)
    target = torch.ones_like(d_real_decision).cuda(0)
    d_real_error = criterion(d_real_decision, target)  # ones = true

    #  1B: Train D on fake
    d_gen_input = torch.from_numpy(noise_sampler(config.minibatch_size, config.g_inp)).cuda(0)

    with torch.no_grad():
        d_fake_data = G(d_gen_input)
    d_fake_decision = D(d_fake_data)
    target = torch.zeros_like(d_fake_decision).cuda(0)
...
def g_loop(config, dset, G, D, g_optimizer, d_optimizer, criterion):
    ...
    gen_input = torch.from_numpy(noise_sampler(config.minibatch_size, config.g_inp)).cuda(0)
    ...
    if config.unrolled_steps > 0:
        if config.use_higher:
            ...
            with higher.innerloop_ctx(D, d_optimizer) as (functional_D, diff_D_optimizer):
                ...
                target = torch.ones_like(dg_fake_decision).cuda(0)
