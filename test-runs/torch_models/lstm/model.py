...
class LSTM(nn.Module):
    ...
    def forward(self, inputs):
        output = []
        c = torch.zeros([1, self.cell.mem_dim]).cuda(0)
        h = torch.zeros([1, self.cell.mem_dim]).cuda(0)
        if self.use_dtr:
...
class LSTMClassifier(nn.Module):
    ...
    def init_hidden(self):
        if self.use_gpu:
            h0 = Variable(torch.zeros(1, self.batch_size, self.hidden_dim).cuda(0))
            c0 = Variable(torch.zeros(1, self.batch_size, self.hidden_dim).cuda(0))
        else:
    ...
    def forward(self, sentence):
        embeds = torch.randint(self.vocab_size - 1, [sentence.size(0), self.batch_size, self.embedding_dim], dtype=torch.float).cuda(0)
...
class GRUClassifier(nn.Module):
    ...
    def forward(self, sentence):
        hidden = Variable(torch.zeros(1, self.batch_size, self.hidden_dim)).cuda(0)
