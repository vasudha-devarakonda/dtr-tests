...
def get_criterion(model: str):
    if model == 'lstm':
        return torch.nn.CrossEntropyLoss().cuda(0)
    if model.startswith('treelstm'):
        return torch.nn.KLDivLoss().cuda(0)
    if model == 'unet':
        return torch.nn.BCEWithLogitsLoss().cuda(0)
    if model == 'unroll_gan':
        return binary_cross_entropy
    if model == 'transformer':
        return torch.nn.NLLLoss().cuda(0)
    return torch.nn.CrossEntropyLoss().cuda(0)
...
def prepare_vision_cnn(stem, model_name, batch_size, use_dtr=False):
    ...
    return prepare_model,  run_model, teardown
...
def prepare_word_language_model(model_name, batch_size, use_dtr=False):
    ...
    if model_name == 'transformer_encoder':
        ...
        return create_model,  run_model, teardown
...
def setup_treelstm_data():
    ...
    if os.path.isfile(emb_file):
        ...
        emb = torch.zeros(vocab.size(), glove_emb.size(1), dtype=torch.float).cuda(0)
        emb.normal_(0, 0.05)
...
def prepare_pythia(model_name, batch_size, use_dtr):
    """
    ...
    def prepare_model(extra_params=None):
        ...
        # Return tokenizer for use outside (encoding, batch prep, etc.)
        return [model]
    ...
    def run_model(model, data, target,
                  ...
                  optimizer=None):
        """
        data, target: torch.LongTensor input_ids already prepared by caller.
        Shape: (batch, seq)
        """
        import torch
        ...
        # Already numeric tensors, just move to GPU
...
def prepare_googlenet(model_name, batch_size, use_dtr):
    def prepare_model(extra_params=None):
...
def prepare_model(model_name, batch_size, use_dtr=False):
    ...
    if model_name.startswith('googlenet'):
        return prepare_googlenet(model_name, batch_size, use_dtr)
    if model_name.startswith('pythia'):
        return prepare_pythia(model_name, batch_size, use_dtr)
