...
from utils_transformer import fetch_dataloaders
...
def run_measurements( i, model_name,
                     n_reps, use_dtr,
                     results_queue, epochs, batch_size, memory_budget, training_loader,tokenizer):
    """
    ...
    def timing_loop(model_name, i,  n_reps,
                    use_dtr,
                    results_queue, batch_size, training_loader, tokenizer):
        measurements = []
        ...
        for e in range(epochs):
            ...
            for j, (inputs_) in enumerate(training_loader):
                gc.collect()
                inputs =  inputs_["input_ids"].cuda(0)
                labels = inputs.clone()
...
def main(experiment_mode, model_name, input_idx, out_file, epochs, batch_size,memory_budget):
    ...
    training_loader,_, tokenizer = fetch_dataloaders("dolly-15k", "EleutherAI/pythia-160m",  batch_size )
    ...
    measurements1 = run_measurements(i, model_name, 
                     remaining_reps, use_dtr, measurements, epochs, batch_size, memory_budget, training_loader, tokenizer)
...
if __name__ == '__main__':
    ...
    def patched_merge_heads(self, tensor, num_attention_heads, attn_head_size):

        tensor = tensor.permute(0, 2, 1, 3).contiguous() 
        tensor = tensor.reshape(tensor.size(0), tensor.size(1), num_attention_heads * attn_head_size)
        return tensor

    GPTNeoXAttention._merge_heads = patched_merge_heads

    invoke_main(main, 'experiment_mode',
                'model_name', 'input_idx',
                'out_file', 'epochs', 'batch_size', "memory_budget")

# python run_transformer.py --experiment_mode dtr --model_name pythia  --input_idx 0 --out_file output.json --epochs 1 --batch_size 1 --memory_budget 5000