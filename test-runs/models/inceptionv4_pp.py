...
class InceptionV4(nn.Module):
    ...
    def __init__(self, A, B, C, k=192, l=224, m=256, n=384, class_nums=100, use_checkpoint=False):

        super().__init__()
        self.use_checkpoint = use_checkpoint
        self.stem = Inception_Stem(3)
        self.inception_a = self._generate_inception_module(384, 384, A, InceptionA, use_checkpoint)
        self.reduction_a = ReductionA(384, k, l, m, n)
        output_channels = self.reduction_a.output_channels
        self.inception_b = self._generate_inception_module(output_channels, 1024, B, InceptionB, use_checkpoint)
        self.reduction_b = ReductionB(1024)
        self.inception_c = self._generate_inception_module(1536, 1536, C, InceptionC, use_checkpoint)
    ...
    @staticmethod
    def _generate_inception_module(input_channels, output_channels, block_num, block, use_checkpoint=False):
        ...
        for l in range(block_num):
            if l%2 == 0:
                ...
                layers.add_module("{}_{}".format(block.__name__, l), block(input_channels, use_checkpoint=use_checkpoint))
...
class InceptionResNetA(nn.Module):
    ...
    def __init__(self, input_channels):
        ...
        self.bn = nn.BatchNorm2d(384,track_running_stats=False)
        self.relu = nn.ReLU(inplace=True)
...
class InceptionResNetB(nn.Module):
    ...
    def __init__(self, input_channels):
        ...
        self.bn = nn.BatchNorm2d(1154,track_running_stats=False)
        self.relu = nn.ReLU(inplace=True)
...
class InceptionResNetC(nn.Module):
    ...
    def __init__(self, input_channels):
        ...
        self.bn = nn.BatchNorm2d(2048,track_running_stats=False)
        self.relu = nn.ReLU(inplace=True)
